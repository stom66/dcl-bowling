...
export namespace ClientMessaging {
	...
	export function requestPlayRoll(position: Vector3, direction: Vector3, power: number): void {
		console.log('ClientMessaging: requestPlayRoll: position', position, 'direction', direction, 'power', power)
		room.send(MessageType.REQUEST_PLAY_ROLL, { position: position, direction: direction, power: power, frameIndex: clientStore.getCurrentFrameIndex() ?? 0, rollIndex: clientStore.getCurrentRollIndex() ?? 0 })
	}
}
